    ...
    public class Formula
    {
        ...
        public Formula(string formula, Func<string, string> normalize, Func<string, bool> isValid)
        {
            ...
            if (Regex.IsMatch(last, opPattern) || last == "(")
                throw new FormulaFormatException("Last token should only be a number, variable, or closing parentheses");
            ...
            foreach (var token in tokens)
            {
                ...
                if (Regex.IsMatch(token, opPattern) || token == ")")
                {
                    if (_substrings.Count == 0)
                        throw new FormulaFormatException("First token of an expression or after an open parentheses should only be a number, variable, or open parentheses");
                    if (prev == "(")
                        throw new FormulaFormatException("Open parentheses is being used before " + token + " which is not allowed");
                    if (Regex.IsMatch(prev, opPattern))
                        throw new FormulaFormatException("Double operator or operator after open parentheses is not allowed");
                ...
                else
                {
                    ...
                    else if (double.TryParse(token, out var tokenDouble))
                    {
            ...
            if (parCount != 0) throw new FormulaFormatException("Not all of the open parentheses in the equation are closed!");
        }
        ...
        public object Evaluate(Func<string, double> lookup)
        {
            ...
            foreach (var sub in _substrings)
            {
                var topOperator = operatorStack.Any() ? operatorStack.Peek() : "";
                // Test if the substring is a number
                if (double.TryParse(sub, out var value))
                {
            ...
            return TryCalculate(valueStack.Pop(), valueStack.Pop(), operatorStack.Pop(), out var calculation)
                ? (object)calculation
