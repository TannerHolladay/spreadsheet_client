    ...
    public static class Evaluator
    {
        ...
        public static int Evaluate(string exp, Lookup variableEvaluator)
        {
            ...
            foreach (string sub in substrings)
            {
                ...
                if (int.TryParse(sub, out int value))
                {
                    ...
                    Regex.IsMatch(sub, "^[a-zA-Z]+[0-9]+$")) //Test if the substring is a value. Ex(a4, ab37, h4, etc..)
                {
            ...
            {
                if (valueStack.Count() == 1) return valueStack.Pop();

                throw new ArgumentException("No values were given in the expression");
            }

            if (valueStack.Count() == 2) return Calculate(valueStack.Pop(), valueStack.Pop(), operatorStack.Pop());
